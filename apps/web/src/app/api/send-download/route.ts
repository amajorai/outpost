...
const DOWNLOAD_URL = "https://github.com/amajorai/outpost/releases/latest";
...
export async function POST(req: Request) {
  ...
  const html = `
    <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;background:#09090b;color:#fff;border-radius:12px">
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:24px">
        <span style="font-weight:600;font-size:16px">Outpost</span>
      </div>
      ...
      <p style="color:#a1a1aa;font-size:14px;margin:0 0 24px">
        Click the button below to download the latest version of Outpost.
      </p>
  ...
  const res = await fetch(`${USESEND_HOST}/api/v1/emails`, {
    ...
    body: JSON.stringify({
      from: "Outpost <no-reply@amajor.ai>",
      to: email,
      subject: "Your Outpost download link",
      html,
    }),
  });
